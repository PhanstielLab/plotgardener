...
## Annotate text at an x-coordinate of 29000000 bp
bb_annoText(plot = signalPlot, label = "locus",
            x = unit(29000000, "native"), y = unit(1, "npc"))
```
...
```{r ideogram_01, eval=TRUE, echo=TRUE, message=FALSE, fig.height=7.25, fig.width=2.5}
## Load cytoband data
data("cytoBand.Hsapiens.UCSC.hg19")
library(TxDb.Hsapiens.UCSC.hg19.knownGene)
bb_pageCreate(width = 0.5, height = 6.25, default.units = "inches",
              showGuides = FALSE, xgrid = 0, ygrid = 0)

bb_plotIdeogram(chrom = "chr1", assembly = "hg19",
                orientation = "v",
                x = unit(0.25, "npc"), y = 0.25, width = 0.3, height = 5.75)
...
1) A United States map

```{r ggplot_01, eval = TRUE, echo = TRUE}
library(ggplot2)
library(scales)
data("bb_DeathsUSA")

US_map <- ggplot(bb_DeathsUSA, aes(long, lat, group = group)) +
  theme_void() +
  ...
  labs(title = "Millions of Deaths") +
  coord_fixed(1.3)

print(US_map)
```

2) Timeline line plots:

```{r ggplot_02, eval = TRUE, echo = TRUE}
data("bb_CasesNYFL")

# Format y-labels
ylabels <- seq(0, 2000000, by = 500000)/1e6
ylabels[c(1,3,5)] <- round(ylabels[c(1,3,5)], digits = 0)
ylabels[c(2,4)] <- round(ylabels[c(2,4)], digits = 1)
ylabels[5] <- paste0(ylabels[5], "M cases")

cases_NYline <- ggplot(bb_CasesNYFL[bb_CasesNYFL$state == "new york",], 
                       aes(x = date, y = cumsum(caseIncrease))) +
  geom_line(size = 0.3) +
  scale_y_continuous(labels = ylabels, position = "right") +
  coord_cartesian(ylim = c(0, 2000000)) +
  theme(panel.background = element_rect(fill = "transparent", color = NA),
        plot.background = element_rect(fill = "transparent", color = NA),
        axis.line.x.bottom = element_line(size = 0.1, color = "grey"),
...
```{r ggplot_03, eval = TRUE, echo = TRUE}
cases_FLline <- ggplot(bb_CasesNYFL[bb_CasesNYFL$state == "florida",], 
                       aes(x = date, y = cumsum(caseIncrease))) +
  geom_line(size = 0.3) +
...
```{r ggplot_04, eval = TRUE, echo = TRUE}
...
vaccines_NYpie <- ggplot(bb_VaccinesNYFL[bb_VaccinesNYFL$state == "new york",],
  ...
  theme_void() +
  scale_fill_manual(values = c("#002b84", "#2196c2", "light grey")) +
  coord_polar(theta = "y", start = 15) +
  geom_text(aes(x = 1.1, label = paste0(percent, "%")),
            color = c("black", "white", "white"),
            size = 2.5,
            position = position_stack(vjust = 0.5)) +
  annotate(geom = "text",
           x = c(1.7, 1.9, 2.1),
           y = c(3e6, 7.5e6, 1.44e7),
           label = c("None", "One", "Two Doses"),
           size = 2.25) +
  theme(legend.position = "none",
        plot.title = element_text(hjust = 0.5, vjust = -3.5, size = 12)) +
  labs(title = "New York")
...
vaccines_FLpie <- ggplot(bb_VaccinesNYFL[bb_VaccinesNYFL$state == "florida",], 
  ...
  scale_fill_manual(values = c("#002b84", "#2196c2", "light grey")) +
  theme_void() +
  coord_polar(theta = "y", start = 15.15) +
  geom_text(aes(x = 1.1, label = paste0(percent, "%")),
            color = c("black", "white", "white"),
            size = 2.5,
            position = position_stack(vjust = 0.5)) +
...
```{r gg_plot05, eval=TRUE,echo=TRUE,message=FALSE,fig.width=9.25, fig.height=4.5}
bb_pageCreate(width = 7.25, height = 3.5, default.units = "inches")

bb_plotGG(plot = US_map,
          x = 0.1, y = 1.625,
          width = 4.6, height = 2.8, just = "left")

bb_plotGG(plot = cases_NYline,
          x = 4.3,  y = 1.75,
          width = 2.75, height = 1.6, just = c("left", "bottom"))
...
bb_plotGG(plot = vaccines_NYpie,
          x = 4.35, y = 0,
          width = 1.25, height = 1.25, just = c("left", "top"))
bb_plotGG(plot = vaccines_FLpie,
          x = 4.35, y = 1.6,
          width = 1.25, height = 1.25, just = c("left", "top"))
```

```{r gg_plot06, eval=TRUE, echo=FALSE, message=FALSE,fig.width=9.25,fig.height=4.5}
bb_pageCreate(width = 7.25, height = 3.5, default.units = "inches")
...
We are then left with a complex, elegant arrangement of `ggplots` as if we had arranged them together with graphic design software:

```{r gg_plot07, eval=TRUE, echo=FALSE, message=FALSE,fig.width=9.25,fig.height=4.5}
bb_pageCreate(width = 7.25, height = 3.5, default.units = "inches",
              showGuides = FALSE, xgrid = 0, ygrid = 0)
...
It is also possible to plot basic shapes and images to further enhance a `BentoBox` layout. These functions include `bb_plotCircle()`, `bb_plotPolygon()`, `bb_plotRaster()`, `bb_plotRect()`, `bb_plotSegments()`, and `bb_plotText()`. 

For example, perhaps we want to create a presentation slide detailing an experimental workflow. Not only can we add `BentoBox` plots based on genomic data:

```{r shapes_01, eval=TRUE, echo=TRUE, message=FALSE, fig.height=5.5, fig.width=10}
data("bb_imrH3K27acData")
data("bb_imrHicData")
bb_pageCreate(width = 8, height = 4.5, default.units = "inches")
bb_plotSignal(data = bb_imrH3K27acData,
              chrom = "chr21", chromstart = 28000000, chromend = 30300000,
              x = 0.25, y = 2, width = 2.25, height = 1, 
              just = c("left", "top"), default.units = "inches")
bb_plotHicSquare(data = bb_imrHicData,
                 chrom = "chr21", chromstart = 28000000, chromend = 30300000,
                 x = 3.25, y = 1.5, width = 2, height = 2,
                 just = c("left", "top"), default.units = "inches")
```

But we can also add some text labels for the title of our slide and the the steps of our workflow:

```{r eval=FALSE, echo=TRUE, message=FALSE}
bb_plotText(label = "EXPERIMENTAL WORKFLOW", fontsize = 24, fontface = "bold",
            x = 0.25, y = 0.5, just = "left")
bb_plotText(label = "H3K27ac", fontsize = 16,
            x = 1.375, y = 4, just = "center", default.units = "inches")
bb_plotText(label = "Hi-C", fontsize = 16,
            x = 4.25, y = 4, just = "center", default.units = "inches")
bb_plotText(label = "Analysis", fontsize = 16,
            x = 6.875, y = 4, just = "center", default.units = "inches")
```
```{r shapes_02, eval=TRUE, echo=FALSE, message=FALSE, fig.height = 5.5, fig.width=10}
bb_pageCreate(width = 8, height = 4.5, default.units = "inches")
...
bb_plotRaster(image = rlogo,
               x = 6, y = 2.5, width = 1.75, height = 1.55,
               just = "left")
```
```{r shapes_03, eval=TRUE, echo=FALSE, message=FALSE, fig.height = 5.5, fig.width=10}
bb_pageCreate(width = 8, height = 4.5, default.units = "inches")
...
Add line segments with arrows to our workflow:
```{r eval=FALSE, echo=TRUE, message=FALSE}
bb_plotSegments(x0 = 2.7, y0 = 2.5, x1 = 3.1, y1 = 2.5, 
                linecolor = "grey", fill = "grey",
                arrow = arrow(angle = 30,
                              length = unit(0.1, "inches"),
                              ends = "last",
                              type = "closed"),
...
bb_plotSegments(x0 = 5.45, y0 = 2.5, x1 = 5.85, y1 = 2.5, 
                linecolor = "grey", fill = "grey",
                arrow = arrow(angle = 30,
                              length = unit(0.1, "inches"),
                              ends = "last",
                ...
                default.units = "inches")
```
```{r shapes_04, eval=TRUE, echo=FALSE, message=FALSE, fig.height = 5.5, fig.width=10}
bb_pageCreate(width = 8, height = 4.5, default.units = "inches")
...
And add a star to highlight a step in our workflow:

```{r eval=FALSE, echo=TRUE, message=FALSE}
bb_plotPolygon(x = c(7.5, 7.6, 7.33, 7.67, 7.4),
               y = c(1.25, 1.55, 1.35, 1.35, 1.55), 
               linecolor = NA, fill = "yellow",
               default.units = "inches")
```
```{r shapes_05, eval=TRUE, echo=FALSE, message=FALSE, fig.height = 5.5, fig.width=10}
bb_pageCreate(width = 8, height = 4.5, default.units = "inches")
...
This makes our final `BentoBox` plot a combination of genomic data and other helpful figure elements:

```{r shapes_06, eval=TRUE, echo=FALSE, message=FALSE, fig.height = 5.5, fig.width=10}
bb_pageCreate(width = 8, height = 4.5, default.units = "inches",
              showGuides = FALSE, xgrid = 0, ygrid = 0)
...
First we can plot our ideogram. We'll remove gridlines from our page to better see the annotations we make:

```{r anno_11, eval=TRUE, echo=TRUE, message=FALSE, fig.width=8.25, fig.height=3.25}
data("cytoBand.Hsapiens.UCSC.hg19")
library(TxDb.Hsapiens.UCSC.hg19.knownGene)

bb_pageCreate(width = 6.25, height = 2.25, default.units = "inches", xgrid = 0, ygrid = 0)
...
Additional fonts for the `fontfamily` argument can be imported with the package `extrafont`. This makes it possible to incorporate more standard fonts like Times New Roman, Arial, etc. into `BentoBox` figures.
...
pileupPlot <- bb_plotBed(data = bb_bedData, 
                         chrom = "chr21", chromstart = 29073000, chromend = 29074000,
                         fill = c("#7ecdbb", "#37a7db"),
                         x = 0.5, y = 0.25, width = 6.5, height = 4.25,
                         baseline = TRUE, baseline.color = "black",
...
```{r eval=FALSE, echo=TRUE}
bedpePlot <- bb_plotBedpeArches(data = bb_bedpeData, 
...
```{r aes_14, eval=TRUE, echo=TRUE, message=FALSE, fig.width=5.25, fig.height=4.25}
data("bb_gmHicData")
data("bb_imrHicData")
