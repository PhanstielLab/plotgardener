...
plotting multi-omic data and how the `bbParams` object and "below" 
...
## Using the `bbParams` object

The `bbParams()` function creates a `bbParams` object that can contain any 
argument from `BentoBox` functions.  

We can recreate and simplify the multi-omic plot above by saving the genomic 
region, left-based x-coordinate, and width into a `bbParams` object:
...
build our multi-omic figure, we can now pass the `bbParams` object into 
...
The `bbParams` object also simplifies the code for making complex multi-omic 
...
`bbParams` object and re-run the code to generate the figure:
...
genomic region we can use `bbParams()` to specify `gene` and `geneBuffer`. 
...
`c(0, maxScore)`. Here we can do this with our `bbParams` object:
