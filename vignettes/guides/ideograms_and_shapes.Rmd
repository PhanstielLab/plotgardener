...
For example, we can include the R logo in our figures:
...
rlogo <- readPNG(system.file("images",
    "rlogo.png",
    package = "plotgardener"
))
...
We saw how to add `ggplots` to plotgardener layouts in the 
vignette [Incorporating ggplots](incorporating_ggplots.html) with the
`plotGG()` function. Beyond customizing the coordinates and dimensions of
`ggplots`, `plotGG()` can also be used to incorporate other `grob` and
`gtable` objects. Thus, `plotgardener` allows us to easily mix and arrange most
