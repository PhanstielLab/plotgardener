...
In addition to its numerous built-in genomic functions, `plotgardener` can size 
and place `ggplots` and other `grid`-based visualizations within a 
`plotgardener` layout. Rather than arranging these plots in a 
relative manner, `plotgardener` can make and place these plots in absolute sizes 
and locations. This makes it simple and intuitive to make complex and combined 
`plotgardener`,`ggplot`, and other `grid` plot arrangements beyond a 
...
on `grid` graphics, it is possible to incorporate them into `plotgardener` 
figure arrangements.
...
heatmap into a `plotgardener` layout with additional figure elements made
by `plotgardener`:

```{r eval=FALSE}
## Create page
pageCreate(
    width = 4.5, height = 5.5, default.units = "inches")

## Plot ComplexHeatmap and add x-axis tick marks
gg_heatmap <- plotGG(
    plot = heatmap,
    x = 0.5, y = 0.5,
...
## Add labels with `plotgardener`
plotText(label = "Simulated Data", 
        x = unit(0.5, "npc"), y = 0.25, fontsize = 14)

plotText(label = c(-2, 0, 2), 
        x = c(0.56, 2.255, 3.93), y = 5.1, fontsize = 8)
plotText(label = "Distance from center", 
        x = unit(0.5, "npc"), y = 5.3, fontsize = 10)

## Hide page guides
pageGuideHide()
```

```{r gg_plot09, echo=FALSE, fig.height=7.5, fig.width=5.5}
...
plotText(label = "Simulated Data", x = unit(0.5, "npc"), y = 0.25,
          fontsize = 14)

plotText(label = c(-2, 0, 2), x = c(0.56, 2.255, 3.93), 
            y = 5.1, fontsize = 8)
plotText(label = "Distance from center", x = unit(0.5, "npc"), 
            y = 5.3, fontsize = 10)
```
