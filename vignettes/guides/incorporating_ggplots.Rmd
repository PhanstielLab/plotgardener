...
For example, let's say we made this heatmap from a random matrix:
...
```{r}
library(grid)
heatmap <- grid.grabExpr(draw(Heatmap(mat)))
```

We can now pass this object into the `plotGG` function and incorporate the 
heatmap into a `plotgardener` layout:

```{r gg_plot09, fig.height=6.5, fig.width=9}
pageCreate(
    width = 6.5, height = 5.5, default.units = "inches")
plotGG(
    plot = heatmap,
    x = 0.25, y = 0.25,
    width = 6, height = 5, just = c("left", "top")
)
```
