...
to the genomic region of data. `bbAnnoGenomeLabel()` can add genome labels 
...
If our genomic region is small enough, `bbAnnoGenomeLabel()` can also be 
used to display the nucleotide sequence of that region. Similar to IGV, 
`bbAnnoGenomeLabel()` will first represent nucleotides as colored boxes:
...
geneTrack <- bbPlotGenes(
    chrom = "chr1", chromstart = 110146000, chromend = 110146050,
    assembly = "hg19",
    x = 0.5, y = 0.25, width = 4, height = 0.75, default.units = "inches"
)
bbAnnoGenomeLabel(plot = geneTrack, x = 0.5, y = 1.01)
```  

At even finer scales, `bbAnnoGenomeLabel()` will then represent nucleotides 
...
`bbAnnoGenomeLabel()` can annotate the genome label along the y-axis:
...
`bbAnnoXaxis()` and `bbAnnoYaxis()` functions. For example, a Manhattan 
...
`bbAnnoXaxis()` and `bbAnnoYaxis()` have similar usages and customizations.
...
`bbAnnoHeatmapLegend()` in both vertical and horizontal orientations. Genomic 
plots that typically require this annotation are Hi-C plots made with 
`bbPlotHicRectangle()`, `bbPlotHicSquare()`, or `bbPlotHicTriangle()`.
...
`bbAnnoPixels()` will only annotate the specified pixels:
...
`bbAnnoDomains()`. For example, if we want to annotate the following domains
...
We can then use `bbAnnoHighlight()` to highlight our genomic region of 
interest (`chr21:28000000-30300000`) with a box of our desired `height`:

```{r eval=FALSE, echo=TRUE, message=FALSE}
region <- bbParams(chrom = "chr21", chromstart = 28000000, chromend = 30300000)
bbAnnoHighlight(
...
`bbAnnoZoomLines()` to add zoom lines from the genomic region we highlighted:
