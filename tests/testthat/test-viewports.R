...
test_that("annotation viewports", {
    ...
    data("IMR90_HiC_10kb")
    bb_pageCreate(width = 3, height = 5, default.units = "inches")
    suppressMessages(hicPlot <- bb_plotHicSquare(
        data = IMR90_HiC_10kb,
        chrom = "chr21",
        chromstart = 28000000, chromend = 30300000,
        assembly = "hg19",
    ...
    suppressMessages(bb_annoHeatmapLegend(plot= hicPlot, x = 2.6, y = 0.5, 
    ...
    genesPlot <- suppressMessages(bb_plotGenes(
            params = paramssmall,
    ...
    expect_equal(BentoBox:::get_annoViewport(genesPlot),
    ...
    expect_equal(BentoBox:::get_annoViewport(hicPlot),
                 hicPlot$outsideVP)
})
