...
## Usage
```{r homePage, message=FALSE, eval=TRUE, echo=FALSE, warning=FALSE}
...
bb_pageCreate(
    width = 7, height = 4.25, default.units = "inches",
...
## Set genomic and dimension parameters
params_a <- bb_params(
...
hicPlot_top <- bb_plotHicSquare(
    data = bb_gmHicData, params = params_a,
    zrange = c(0, 200), resolution = 10000,
    half = "top",
    y = 0.5, height = 2.75
)
hicPlot_bottom <- bb_plotHicSquare(
    data = bb_imrHicData, params = params_a,
...
bb_annoHeatmapLegend(
    plot = hicPlot_bottom, fontsize = 7,
    x = 3.05, y = 0.5,
    width = 0.07, height = 0.5, just = c("left", "top"),
    default.units = "inches"
)
...
## Plot gene track
genes_a <- bb_plotGenes(
    params = params_a, stroke = 1, fontsize = 6,
    y = 3.35, height = 0.4
)

## Annotate genome label
bb_annoGenomeLabel(
    plot = genes_a, params = params_a,
    scale = "Mb", fontsize = 7,
    y = 3.85
)
...
bb_plotText(
    label = "Chromosome 21", fontsize = 8, fontcolor = "darkgrey",
...
## Set genomic and dimension parameters
params_c <- bb_params(
    chrom = "chr21", chromstart = 28150000, chromend = 29150000, assembly = "hg19",
...
## Plot Hi-C triangle
hic_gm <- bb_plotHicTriangle(
    data = bb_gmHicData, params = params_c,
    zrange = c(0, 200), resolution = 10000,
...
bb_annoHeatmapLegend(
    plot = hic_gm, fontsize = 7,
    x = 5, y = 1, width = 0.07, height = 0.5,
    just = c("right", "top"), default.units = "inches"
)

## Plot CTCF signal
ctcf_gm <- bb_plotSignal(
    data = bb_gmCTCFData, params = c(params_c, ctcf_range),
    fill = "#253494", linecolor = "#253494",
    y = 1.95, height = 0.6
)
## CTCF label
bb_plotText(
    label = "CTCF", fontcolor = "#253494", fontsize = 8,
    x = 3.5, y = 1.95, just = c("left", "top"), default.units = "inches"
)

## Plot H3K27ac signal
hk_gm <- bb_plotSignal(
    data = bb_gmH3K27acData, params = c(params_c, hk_range),
    fill = "#37a7db", linecolor = "#37a7db",
    y = 3.25, height = 0.6, just = c("left", "bottom")
)
## H3K27ac label
bb_plotText(
    label = "H3K27ac", fontcolor = "#37a7db", fontsize = 8,
    x = 3.5, y = 2.65, just = c("left", "top"), default.units = "inches"
)

## Plot genes
genes_gm <- bb_plotGenes(
    params = params_c, stroke = 1, fontsize = 6,
    strandLabels = FALSE,
    y = 3.35, height = 0.4
)
...
## Plot Hi-C triangle
hic_imr <- bb_plotHicTriangle(
    data = bb_imrHicData, params = params_d,
    zrange = c(0, 70), resolution = 10000,
...
## Annotate Hi-C heatmap legend
bb_annoHeatmapLegend(
    plot = hic_imr, fontsize = 7,
    x = 6.75, y = 1, width = 0.07, height = 0.5, just = c("right", "top")
)

## Plot CTCF signal
ctcf_imr <- bb_plotSignal(
    data = bb_imrCTCFData, params = c(params_d, ctcf_range),
    fill = "#253494", linecolor = "#253494",
    y = 1.95, height = 0.6, just = c("right", "top")
)

## Plot H3K27ac signal
hk_imr <- bb_plotSignal(
    data = bb_imrH3K27acData, params = c(params_d, hk_range),
    fill = "#37a7db", linecolor = "#37a7db",
    y = 3.25, height = 0.6, just = c("right", "bottom")
)

## Plot gene track
genes_imr <- bb_plotGenes(
    params = params_d, stroke = 1, fontsize = 6,
    strandLabels = FALSE,
...
## Annotate genome label
bb_annoGenomeLabel(
    plot = genes_imr, params = params_d,
    scale = "Kb", fontsize = 7,
...
## Set genomic and dimension parameters in a `bb_params` object
params_c <- bb_params(
    chrom = "chr21", chromstart = 28150000, chromend = 29150000,
    assembly = "hg19",
    x = 3.5, width = 1.5, default.units = "inches"
)
## Set signal track data ranges
