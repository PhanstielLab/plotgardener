...
#' bbPageCreate(width = 3.75, height = 3.5, default.units = "inches")
#'
...
#' bbPageGuideHide()
#' @export
bbPagePlotPlace <- function(plot, x = NULL, y = NULL, width = NULL,
    ...
    copy_grobs <- function(grob) {
        ...
        assign("new_gtree",
            addGrob(
                gTree = get("new_gtree", envir = bbEnv),
                child = grob
            ),
            envir = bbEnv
        )
    ...
    bb_place <- parseParams(params = params, 
                            ...
                            class = "bb_place")
    ## Justification
    bb_place$just <- bb_justConversion(bb_place$just)
    ...
    object <- set_values(
        object = object, x = bb_place$x, y = bb_place$y,
