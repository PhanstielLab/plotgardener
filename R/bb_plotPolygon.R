...
#' bb_plotPolygon(x = c(2.6, 4.65, 4.75, 6.05, 1.4, 1.3),
#'                y = c(2.5, 3.1, 3.5, 4, 3.15, 2.8),
#'                fill = "#4a168e", linecolor = NA)
#'
#' bb_plotPolygon(x = c(4.65, 4.75, 6.05, 5.05, 4.4),
...
#'
#' @export
bb_plotPolygon <- function(x, y, default.units = "inches",
  ...
  bb_polygonInternal <- structure(list(x = x, y = y, id = id,
  ...
  bb_polygonInternal <- parseParams(bb_params = params,
  ...
  ## Set gp
  bb_polygonInternal$gp <- gpar(col = bb_polygonInternal$linecolor,
                                fill = bb_polygonInternal$fill,
                                lwd = bb_polygonInternal$lwd,
                               lty = bb_polygonInternal$lty,
                               alpha = bb_polygonInternal$alpha)
  bb_polygonInternal$gp <- setGP(gpList = bb_polygonInternal$gp,
  ...
  bb_polygon <- structure(list(x = bb_polygonInternal$x,
  ...
  if(is.null(bb_polygon$x)) stop("argument \"x\" is missing, with no default.",
                                 call. = FALSE)
  if(is.null(bb_polygon$y)) stop("argument \"y\" is missing, with no default.",
                                 call. = FALSE)
  ...
  if (!"unit" %in% class(bb_polygon$y)){
    ...
    if (all(grepl("b", bb_polygon$y)) == TRUE){
      ...
      bb_polygon$y <- unit(unlist(lapply(bb_polygon$y, plot_belowY)),
