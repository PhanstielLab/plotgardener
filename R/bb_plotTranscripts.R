...
#'
#' ## Plot a legend
#' bb_plotLegend(legend = c("+ strand", "- strand"),
...
bb_plotTranscripts <- function(chrom, chromstart = NULL, chromend = NULL,
  ...
  bb_transcriptsInternal <- structure(list(assembly = assembly, chrom = chrom,
  ...
  bb_transcriptsInternal <- parseParams(bb_params = params,
  ...
  bb_transcripts <- structure(list(chrom = bb_transcriptsInternal$chrom,
                                   ...
                                   grobs = NULL), class = "bb_transcripts")
  ...
  bb_transcripts <- defaultUnits(object = bb_transcripts,
  ...
  if(!"unit" %in% class(bb_transcriptsInternal$boxHeight)){
    ...
    bb_transcriptsInternal$boxHeight <- unit(bb_transcriptsInternal$boxHeight,
  ...
  orgdbChecks <- check_loadedPackage(package = bb_transcripts$assembly$OrgDb,
                                     message = paste(paste0("`",
                                                            bb_transcripts$assembly$OrgDb,
                                                            "`"),
                                                     "not loaded. Please install and load to plot gene transcripts."))
  ...
  if (txdbChecks == TRUE & orgdbChecks == TRUE){
    ...
    if (!bb_transcripts$chrom %in% names(genome)){
      ...
      data <- bb_getExons(assembly = bb_transcripts$assembly,
  ...
  vp_name <- paste0("bb_transcripts",
                    length(grep(pattern = "bb_transcripts",
                                x = currentViewports)) + 1)

  if (is.null(bb_transcripts$x) & is.null(bb_transcripts$y)){

    height <- 0.5
    yscale <- strand_scale(strandSplit = bb_transcriptsInternal$strandSplit,
                           height = height)
  ...
  backgroundGrob <- rectGrob(gp = gpar(fill = bb_transcriptsInternal$bg,
  ...
  if (is.null(bb_transcripts$x) & is.null(bb_transcripts$y)){
    ...
    textHeight <- convertHeight(textHeight, unitTo = "npc", valueOnly = TRUE)
    upViewport()
    unit <- "npc"
  ...
  if (bb_transcriptsInternal$strandSplit == FALSE){
    ...
    if (nrow(data) > 0){
      ...
      repData <- defaultGenePriorities(data = repData,
      ...
      rowData <- suppressMessages(dplyr::left_join(x = data,
                                                   y = rowData,
                                                   by = "TXNAME"))
      ...
      if (any(rowData$row == 0)){
        ...
        assign("transcript_grobs",
               addGrob(gTree = get("transcript_grobs", envir = bbEnv),
                       child = limitGrob), envir = bbEnv)
    ...
    if (nrow(posStrand) > 0){
      ...
      ## Access default transcript prioritization based on citation/transcript length
      repPosData <- defaultGenePriorities(data = repPosData,
                                          assembly = bb_transcripts$assembly,
                                          transcript = TRUE)
      ...
      posRowData <- suppressMessages(dplyr::left_join(x = posStrand,
                                                      y = posRowData,
                                                      by = "TXNAME"))
    ...
    if (nrow(minStrand) > 0){
      ...
      ## Access default transcript prioritization based on citation/transcript length
      repMinData <- defaultGenePriorities(data = repMinData,
                                          assembly = bb_transcripts$assembly,
                                          transcript = TRUE)
      ## Convert to numeric matrix with txstart, txend, and row for Rcpp function parsing
      ...
      minRowData <- suppressMessages(dplyr::left_join(x = minStrand,
                                                      y = minRowData,
                                                      by = "TXNAME"))
  ...
  if (bb_transcriptsInternal$colorbyStrand == FALSE
    ...
    colors <- rep(bb_transcriptsInternal$fill,
                  ceiling(maxRows/length(bb_transcriptsInternal$fill)))[seq(1,maxRows)]
    indeces <- rowData$row
  ...
  if (nrow(rowData) > 0){
    ...
    if ((bb_transcripts$chromend - bb_transcripts$chromstart) >= 25000000){
      ...
      assign("transcript_grobs",
             addGrob(get("transcript_grobs", envir = bbEnv),
                     child = transcriptExons), envir = bbEnv)
      ...
      if (nrow(cdsData) > 0){
        ...
        transcriptCds <- rectGrob(x = cdsData$CDSSTART,
                                  ...
                                  height = boxHeight,
                                  just = "left",
                                  gp = gpar(fill = cdsData$color,
                                            col = cdsData$color,
                                            lwd = 1.25),
                                  default.units = "native")
        assign("transcript_grobs",
               addGrob(get("transcript_grobs", envir = bbEnv),
                       child = transcriptCds), envir = bbEnv)
    ...
    assign("transcript_grobs",
           addGrob(get("transcript_grobs", envir = bbEnv),
                   child = transcriptLine), envir = bbEnv)
    ...
    if (!is.null(bb_transcriptsInternal$labels)){
      ...
      if (bb_transcriptsInternal$labels == "transcript"){
        ...
        label <- paste0(transcriptNames[[bb_transcripts$assembly$display.column]],
                        ":", transcriptNames$TXNAME)
      }

      ## Get which names aren't cutoff
      checkedLabels <- apply(data.frame("label" = label, "labelLoc" = transcriptNames$labelLoc),
                             1, cutoffLabel,
      ...
      if (nrow(transcriptNames) > 0){
        ...
        transcript_names <- textGrob(label = transcriptNames$label,
                                    ...
                                    just = "bottom",
                                    gp = gpar(col = transcriptNames$color,
                                              fontsize = bb_transcriptsInternal$fontsize),
                                    default.units = "native",
                                    check.overlap = TRUE)
        ...
        assign("transcript_grobs",
               addGrob(get("transcript_grobs", envir = bbEnv),
                       child = transcript_names), envir = bbEnv)
