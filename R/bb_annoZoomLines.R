...
#' @param params An optional \link[BentoBox]{bb_params}
...
#' zoomRegion <- bb_params(chrom = "chr21",
...
#' manhattanPlotZoom <- bb_plotManhattan(data = bb_gwasData, fill = "grey",
#'                                       sigLine = FALSE,
#'                                       baseline = TRUE,
...
#' signalPlot <- bb_plotSignal(data = bb_imrH3K27acData, params = zoomRegion,
#'                             range = c(0, 45),
#'                             x = 0.5, y = "b0.1",
...
#'
#' @export
bb_annoZoomLines <- function(plot, chrom, chromstart = NULL, chromend = NULL,
  ...
  bb_zoomInternal <- structure(list(plot = plot, chrom = chrom,
  ...
  bb_zoomInternal <- parseParams(bb_params = params,
  ...
  ## Set gp
  bb_zoomInternal$gp <- gpar(col = bb_zoomInternal$linecolor,
                             lty = bb_zoomInternal$lty)
  bb_zoomInternal$gp <- setGP(gpList = bb_zoomInternal$gp,
  ...
  bb_zoom <- structure(list(chrom = bb_zoomInternal$chrom,
  ...
  if(is.null(bb_zoom$chrom)) stop("argument \"chrom\" is missing, with no default.",
                                  call. = FALSE)
  if(is.null(bb_zoom$y0)) stop("argument \"y0\" is missing, with no default.",
                               call. = FALSE)
  if(is.null(bb_zoom$y1)) stop("argument \"y1\" is missing, with no default.",
                               call. = FALSE)
  ...
  if (!is.null(bb_zoom$chromstart) & !is.null(bb_zoom$chromend)){
    if (class(bb_zoomInternal$plot) == "bb_manhattan"){
      ...
      if (length(bb_zoomInternal$plot$chrom) > 1){
        ...
        convertedCoords <- convertManhattan(object = bb_zoom,
  ...
  if(!is.null(bb_zoom$chromstart) & !is.null(bb_zoom$chromend)){
    ...
    assign("zoom_grobs",
           addGrob(gTree = get("zoom_grobs", envir = bbEnv),
                   child = zoomSegment_left), envir = bbEnv)
    assign("zoom_grobs",
           addGrob(gTree = get("zoom_grobs", envir = bbEnv),
                   child = zoomSegment_right), envir = bbEnv)
    ...
    extend_topLeft <- segmentsGrob(x0 = bb_zoom$x0[1],
                                   y0 = unit(page_height, page_units) - bb_zoom$y0[1],
                                   x1 = bb_zoom$x0[1],
                                   y1 = topy1_left,
                                   gp = bb_zoomInternal$gp)
    extend_topRight <- segmentsGrob(x0 = bb_zoom$x0[2],
                                    y0 = unit(page_height, page_units) - bb_zoom$y0[2],
                                    x1 = bb_zoom$x0[2],
                                    y1 = topy1_right,
                                    gp = bb_zoomInternal$gp)
    extend_bottomLeft <- segmentsGrob(x0 = bb_zoom$x1[1],
                                      y0 = unit(page_height, page_units) - bb_zoom$y1[1],
                                      x1 =bb_zoom$x1[1],
                                      y1 = bottomy1_left,
                                      gp = bb_zoomInternal$gp)
    extend_bottomRight <- segmentsGrob(x0 = bb_zoom$x1[2],
    ...
    assign("zoom_grobs",
           addGrob(gTree = get("zoom_grobs", envir = bbEnv),
                   child = extend_topLeft), envir = bbEnv)
    assign("zoom_grobs",
           addGrob(gTree = get("zoom_grobs", envir = bbEnv),
                   child = extend_topRight), envir = bbEnv)
    assign("zoom_grobs",
           addGrob(gTree = get("zoom_grobs", envir = bbEnv),
                   child = extend_bottomLeft), envir = bbEnv)
    assign("zoom_grobs",
           addGrob(gTree = get("zoom_grobs", envir = bbEnv),
                   child = extend_bottomRight), envir = bbEnv)
