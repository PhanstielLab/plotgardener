...
#'
#' @export
bb_pageCreate <- function(width = 8.5, height = 11, default.units = "inches",
  ...
  errorcheck_bb_pageCreate <- function(object){
    ...
    if (widthUnit != heightUnit){
      ...
      stop(paste("`width` and `height` must be in the same units. `width` detected as",
                 paste0("`",widthUnit,"`"),
                 "and `height` detected as",
                 paste0(paste0("`",heightUnit,"`"), ".")), call. = FALSE)
    }

    ## Page units must be a valid option
    page_units <- gsub("[0-9]|[.]", "", bb_page$width)
    if (!page_units %in% validUnits){
      stop(paste(c("Invalid page units. Options for page units are:", validUnits),
                 collapse = " "), call. = FALSE)
    }
  ...
  bb_page <- structure(list(width = width, height = height,
  ...
  if (!"unit" %in% class(bb_page$width)){
    ...
    if (!bb_page$default.units %in% validUnits){
      stop(paste(c("Invalid default units. Options for page units are:", validUnits),
                 collapse = " "), call. = FALSE)
    }
  ...
  if (bb_page$showGuides == TRUE){
    ...
    if (page_units == "inches"){
      ...
      for (i in seq(1,4)){
        ...
        y0 <- y1
        y1 <- y1 + 1/32
        v <- segmentsGrob(x0 = xsegs$x0[xsegs$x0 %in% seq(0, page_width, div)],
                          y0 = y0,
                          x1 = xsegs$x0[xsegs$x0 %in% seq(0, page_width, div)],
                          y1 = y1, default.units = page_units,
                          gp = gpar(col = "black"))
        h <- segmentsGrob(x0 = x0,
      ...
      hLabel <- textGrob(label = seq(0, page_width, div),
                         x = seq(0, page_width, div),
                         y = y0, vjust = -0.5,
                         default.units = page_units)
      vLabel <- textGrob(label = seq(0, page_height, div),
                         x = x0,
      ...
      tickH <- convertUnit(x = unit(1/32, "inches"),
      ...
      xsegs <- segmentsGrob(x0 = seq(0, page_width, div),
                            y0 = y0,
                            x1 = seq(0, page_width, div),
                            y1 = y1,
                            default.units = page_units,
                            gp = gpar(col = "black"))
      ysegs <- segmentsGrob(x0 = x0,
      ...
      assign("guide_grobs",
             addGrob(gTree = get("guide_grobs", envir = bbEnv),
                     child = v2), envir = bbEnv)
      assign("guide_grobs",
             addGrob(gTree = get("guide_grobs",
                                 envir = bbEnv), child = h2), envir = bbEnv)

      div3 <- 1
      v3 <- segmentsGrob(x0 = seq(0, page_width, div3),
                         y0 = page_height + 4*tickH,
                         x1 = seq(0, page_width, div3),
                         y1 = page_height + 5*tickH,
                         default.units = page_units,
                         gp = gpar(col = "black"))
      h3 <- segmentsGrob(x0 = -4*tickH,
      ...
      assign("guide_grobs",
             addGrob(gTree = get("guide_grobs", envir = bbEnv),
                     child = v3), envir = bbEnv)
      assign("guide_grobs",
             addGrob(gTree = get("guide_grobs", envir = bbEnv),
                     child = h3), envir = bbEnv)

      hLabel <- textGrob(label = seq(0, page_width, div3),
                         x = seq(0, page_width, div3),
                         y = page_height + 4*tickH,
                         vjust = -0.5,
                         default.units = page_units)
      vLabel <- textGrob(label = seq(0, page_height, div3),
                         x = -4*tickH,
      ...
      assign("guide_grobs",
             addGrob(gTree = get("guide_grobs",
                                 envir = bbEnv), child = h3), envir = bbEnv)
    ...
    assign("guide_grobs",
           addGrob(gTree = get("guide_grobs", envir = bbEnv),
                   child = xsegs), envir = bbEnv)
    assign("guide_grobs",
           addGrob(gTree = get("guide_grobs", envir = bbEnv),
                   child = ysegs), envir = bbEnv)
    assign("guide_grobs",
           addGrob(gTree = get("guide_grobs", envir = bbEnv),
                   child = hLabel), envir = bbEnv)
    assign("guide_grobs",
           addGrob(gTree = get("guide_grobs", envir = bbEnv),
                   child = vLabel), envir = bbEnv)


    ## Unit annotation
    unitLabel <- textGrob(label = page_units, x = 0,
                          y = page_height, hjust = 1.75,
                          vjust = -1.5, default.units = page_units,
                          just = c("right", "bottom"))
    assign("guide_grobs",
           addGrob(gTree = get("guide_grobs", envir = bbEnv),
                   child = unitLabel), envir = bbEnv)
  ...
  tryCatch(expr = {
    ...
    xGrid <- segmentsGrob(x0 = seq(0, page_width, bb_page$xgrid),
                          y0 = 0,
                          x1 = seq(0, page_width, bb_page$xgrid),
                          y1 = bb_page$height,
                          default.units = page_units,
                          gp = gpar(col = "grey50", lty = 2, lwd = 0.5))

    yGrid <- segmentsGrob(x0 = 0,
                          y0 = seq(0, page_height, bb_page$ygrid),
                          x1 = page_width,
                          y1 = seq(0, page_height, bb_page$ygrid),
                          default.units = "native",
                          gp = gpar(col = "grey50", lty = 2, lwd = 0.5))

    assign("guide_grobs",
           addGrob(gTree = get("guide_grobs", envir = bbEnv),
                   child = xGrid), envir = bbEnv)
    assign("guide_grobs",
           addGrob(gTree = get("guide_grobs", envir = bbEnv),
                   child = yGrid), envir = bbEnv)
