...
#' chr11_manhattanPlot <- bbPlotManhattan(
...
#'
#' @export
bbPlotManhattan <- function(data, sigVal = 5e-08, chrom = NULL,
                            chromstart = NULL, chromend = NULL,
    ...
    ## Define a function that checks for errors in bbPlotManhattan
    errorcheck_bbPlotmanhattan <- function(bedfile, chrom, chromstart,
                                            chromend, object,
                                            leadSNP, fill) {
    ...
    ## Justification
    bb_manInternal$just <- bb_justConversion(just = bb_manInternal$just)
