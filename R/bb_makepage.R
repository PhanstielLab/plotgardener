...
#'
#' @param width page width, in specified units
#' @param height page height, in specified units
...
bb_makePage <- function(width = 8.5, height = 11, units = "inches", showOutline = TRUE, showRuler = TRUE, xgrid = 0.5, ygrid = 0.5){
  ...
  if (showRuler == TRUE){
    ...
    hLabel <- grid.text(label = seq(0, width, div), x = seq(0, width, div), y = y0, vjust = -0.5, default.units = units)
    vLabel <- grid.text(label = seq(0, height, div), x = x0, y = seq(0, height, div), hjust = 1.5, default.units = "native")

    ## Unit annotation
    unitLabel <- grid.text(label = units, x = 0, y = height, hjust = 1.75, vjust = -1.5, default.units = units, just = c("right", "bottom"))
