...
bb_addscale <- function(color_vector, min_val, max_val, border = FALSE, height, width, x, y, units = "inches", orientation = "vertical", fontsize = 8,
  ...
  errorcheck_bb_addscale <- function(bb_scale){
    ...
    if (as.numeric(bb_legend$min_val) > as.numeric(bb_legend$max_val)){
    ...
    if (!bb_legend$orientation %in% c("vertical", "horizontal")){
  ...
  bb_scale <- structure(list(color_palette = color_vector, min_val = min_val, max_val = max_val,
                              orientation = orientation, height = height, width = width, x = x, y = y, units = units), class = "bb_scale")

  # ======================================================================================================================================================================================
  # CALL ERRORS
  # ======================================================================================================================================================================================

  errorcheck_bb_addlegend(bb_scale = bb_scale)
  ...
  if (orientation == "vertical"){
    ...
    grid.raster(rev(color_vector), width = unit(page_coords[[1]]$width, page_coords[[3]]), height = unit(new_height, "npc"), y = 1 - (hH + (0.5 * dH)), just = "top")

    if (border == T){

      grid.rect(y = 1 - (hH + (0.5 * dH)), just = "top", width = unit(page_coords[[1]]$width, page_coords[[3]]), height = unit(new_height, "npc"),
  ...
  if (orientation == "horizontal"){
    ...
    grid.raster(matrix(data = color_vector, nrow = 1, ncol = length(color_vector)), width = unit(new_width, "npc"), height = unit(page_coords[[1]]$height, page_coords[[3]]),
                x = lW + (0.5 * dW), just = "left")

    if (border == T){

      grid.rect(x = lW + (0.5 * dW), just = "left", width = unit(new_width, "npc"), height = unit(page_coords[[1]]$height, page_coords[[3]]),
                gp = gpar(col = "black", fill = NA) )
