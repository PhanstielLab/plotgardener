...
bb_plotSignal <- function(data, binSize = NA, binCap = TRUE, negData = FALSE,
    ...
    format_data <- function(signal, signaltrack) {
        ...
        signal <- signal[
            which(signal[, 1] == signaltrack$chrom &
                ((signal[, 2] > signaltrack$chromstart &
                    signal[, 2] < signaltrack$chromend |
                    signal[, 3] > signaltrack$chromstart &
                        signal[, 3] < signaltrack$chromend))),
            (2:4)
        ]
    ...
    sigGrob <- function(signal, fillCol, lineCol, gp) {
        ...
        if (!is.null(fillCol) & !is.na(fillCol)) {
            ...
            sigGrob <- segmentsGrob(
                x0 = signal[seq(1, length(signal[, 1]) - 1), 1],
        ...
        assign("signal_grobs",
            addGrob(
                gTree = get("signal_grobs", envir = bbEnv),
                child = sigGrob
            ),
            envir = bbEnv
        )
    ...
    if (!is.na(signal_track$binSize)) {
        if (split == TRUE) {
            ...
            lineGrob <- segmentsGrob(
                ...
                y0 = 0, y1 = 0,
                gp = gpar(
                    col = bb_sigInternal$baseline.color,
            ...
            if (nrow(posSignal) >= 2) {
                ...
                sigGrob(
                    signal = posSignal2, fillCol = bb_sigInternal$fill[1],
