...
#' \link[GenomicRanges]{GRanges} object with metadata column \code{counts}.
...
#'
#' @return Returns a \code{bb_signal} object containing relevant
...
#'
#' @details
#' #A signal track can be placed on a BentoBox coordinate page
...
bb_plotSignal <- function(data, binSize = NA, binCap = TRUE, negData = FALSE,
  ...
  read_signal <- function(signal, signaltrack){
    ...
    if (!"data.frame" %in% class(signal)){
      ...
      if (!"GRanges" %in% class(signal)){
        signal <- bb_readBigwig(file = signal, chrom = signaltrack$chrom,
  ...
  format_data <- function(signal, signaltrack){
    ...
    if (!any(colnames(signal) == "score")){
      ...
      signal <- signal[c(1,2,3, scoreCol)]
    ...
    if (!is.na(signaltrack$binSize)){
      ...
      if (!is.nan(binNum)){
        ...
        if (binNum > 8000 && binCap == TRUE){
          ...
          signaltrack$binSize <- updated_binSize
          warning(paste0("Too many bins: adjusting to 8000 bins of size ",
        ...
        if (binNum > (signaltrack$chromend - signaltrack$chromstart)){
          ...
          signaltrack$binNum <- updated_binNum
          signaltrack$binSize <- updated_binSize
          warning(paste0("Number of bins larger than plot length: adjusting to ",
                         binNum, " bins of size 1."), call. = FALSE)
        }
  ...
  parseData <- function(signal, signaltrack){
    ...
    if (!is.na(signaltrack$binSize)){
      ...
      binDF <- data.frame("start" = seq(signaltrack$chromstart,
                                        binChromend - signaltrack$binSize,
                                        signaltrack$binSize),
                          "end" = seq(signaltrack$chromstart + signaltrack$binSize,
                                      binChromend,
                                      signaltrack$binSize))
      ...
      linking_regions <- cbind(newSignal[seq(1,(nrow(newSignal) - 1)), 2],
                               newSignal[seq(2,nrow(newSignal)), 1])
      ...
      if (nrow(linking_regions) > 0){
        ...
        colnames(linking_regions)[(seq(1,3))] <- c("chromstart", "chromend", "counts")
      ...
      newSignal <- cbind(as.vector(t(newSignal[,c(1, 2)])),
                         as.vector(t(newSignal[,c(3, 3)])))
  ...
  sigGrob <- function(signal, fillCol, lineCol, gp){
    ...
    if (!is.null(fillCol) & !is.na(fillCol)){
      ...
      sigGrob <- polygonGrob(x = c(signal[1,1], signal[,1],
                                   signal[nrow(signal),1]),
                             y = c(0, signal[,2], 0), gp = gp,
                             default.units = "native")

    } else {
      sigGrob <- segmentsGrob(x0 = signal[seq(1,length(signal[,1])-1), 1],
                              y0 = signal[seq(1,length(signal[,2])-1), 2],
                              x1 = signal[seq(2,length(signal[,1])), 1],
                              y1 = signal[seq(2,length(signal[,2])), 2],
    ...
    ## Add grob to gtree
    assign("signal_grobs",
           addGrob(gTree = get("signal_grobs", envir = bbEnv),
  ...
  cutoffGrobs <- function(signal, signaltrack, side){
    ...
    grobCutoffs <- function(df, side){
      ...
      cutoffGrob <- segmentsGrob(x0 = x0, x1 = x1, y0 = unit(y, "npc"),
                                 y1 = unit(y, "npc"),
                                 gp = gpar(lwd = 1, col = "grey"),
                                 default.units = "native")
      assign("signal_grobs",
             addGrob(gTree = get("signal_grobs", envir = bbEnv),
                     child = cutoffGrob), envir = bbEnv)
  ...
  bb_sigInternal <- structure(list(data = data, chrom = chrom,
  ...
  ## Set gp
  bb_sigInternal$gp <- setGP(gpList = bb_sigInternal$gp,
  ...
  signal_track <- structure(list(chrom = bb_sigInternal$chrom,
                                 chromstart = bb_sigInternal$chromstart,
                                 chromend = bb_sigInternal$chromend,
                                 assembly = bb_sigInternal$assembly,
                                 binSize = bb_sigInternal$binSize,
                                 binNum = NULL, range = bb_sigInternal$range,
                                 ymax = bb_sigInternal$ymax,
                                 x = bb_sigInternal$x, y = bb_sigInternal$y,
                                 width = bb_sigInternal$width,
                                 height = bb_sigInternal$height,
                                 just = bb_sigInternal$just, grobs = NULL),
                            class = "bb_signal")
  ...
  signal_track <- defaultUnits(object = signal_track,
                               default.units = bb_sigInternal$default.units)
  ...
  signal_track <- check_binNum(signaltrack = signal_track,
                               binCap = bb_sigInternal$binCap)
  ...
  if (split == TRUE){
    ...
    signal_track <- set_range(signal1 = posSignal, signal2 = posSignal2,
    ...
    signal_track <- set_range(signal1 = negSignal, signal2 = negSignal2,
                              signaltrack = signal_track, split = TRUE,
                              pos = FALSE)
  ...
  vp_name <- paste0("bb_signal",
                    length(grep(pattern = "bb_signal",
  ...
  backgroundGrob <- rectGrob(gp = gpar(fill = bb_sigInternal$bg,
  ...
  if (!is.na(signal_track$binSize)){
    ...
    if (split == TRUE){
      ...
      if (nrow(posSignal) >= 2){
        sigGrob(signal = posSignal2, fillCol = fills[[1]],
                lineCol = lines[[1]], gp = bb_sigInternal$gp)
        ## Find and make cutoff lines
        cutoffGrobs(signal = posSignal2, signaltrack = signal_track,
                    side = "top")

      } else {
        bb_sigInternal$gp$col <- lines[[1]]
        posGrob <- segmentsGrob(x0 = 0, y0 = unit(0, "native"),
                                x1 = 1, y1 = unit(0, "native"),
                                gp = bb_sigInternal$gp)
        assign("signal_grobs",
               addGrob(gTree = get("signal_grobs", envir = bbEnv),
                       child = posGrob), envir = bbEnv)
        warning("Not enough top signal data to plot.", call. = FALSE)
      }


      if (nrow(negSignal) >= 2){
        sigGrob(signal = negSignal2, fillCol = fills[[2]],
                lineCol = lines[[2]], gp = bb_sigInternal$gp)
        ## Find and make cutoff lines
        cutoffGrobs(signal = negSignal2, signaltrack = signal_track,
                    side = "bottom")
      } else {
        bb_sigInternal$gp$col <- lines[[2]]
        negGrob <- segmentsGrob(x0 = 0, y0 = unit(0, "native"),
                                x1 = 1, y1 = unit(0, "native"),
                                gp = bb_sigInternal$gp)
        assign("signal_grobs",
               addGrob(gTree = get("signal_grobs", envir = bbEnv),
                       child = negGrob), envir = bbEnv)
      ...
      lineGrob <- segmentsGrob(x0 = unit(0, "npc"), x1 = unit(1, "npc"),
                               y0 = 0, y1 = 0,
                               gp = gpar(col = bb_sigInternal$baseline.color,
                                         lwd = bb_sigInternal$baseline.lwd),
                               default.units = "native")
      assign("signal_grobs",
             addGrob(gTree = get("signal_grobs", envir = bbEnv),
                     child = lineGrob), envir = bbEnv)
      ...
      if (nrow(posSignal) >= 2){
        ...
        sigGrob(signal = posSignal2, fillCol = bb_sigInternal$fill[1],
                lineCol = bb_sigInternal$linecolor[1],
                gp = bb_sigInternal$gp)
        ## Find and make cutoff lines
        cutoffGrobs(signal = posSignal2, signaltrack = signal_track, side = "top")
      } else {
        bb_sigInternal$gp$col <- bb_sigInternal$linecolor
        signalGrob <- segmentsGrob(x0 = 0, y0 = unit(0, "native"),
                                   x1 = 1, y1 = unit(0, "native"),
                                   gp = bb_sigInternal$gp)
        assign("signal_grobs",
               addGrob(gTree = get("signal_grobs", envir = bbEnv),
                       child = signalGrob), envir = bbEnv)
    ...
    if (bb_sigInternal$scale == TRUE){
      ...
      scaleGrob <- textGrob(label = paste0("[", lowerLim, " - ", upperLim, "]"),
