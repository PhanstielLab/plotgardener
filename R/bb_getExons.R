bb_getExons <- function(assembly, chromosome, start, stop){
  ...
  txs_wo_symbols <- suppressMessages(AnnotationDbi::select(tx_db, keys = genes_in_range$gene_id, columns = columns(tx_db), keytype = "GENEID"))
  ...
  if (assembly$gene.id.column == assembly$display.column){
    ...
    gene_symbols <- suppressMessages(AnnotationDbi::select(org_db, keys = sub("-.+", "", genes_in_range$gene_id), columns = assembly$display.column, keytype = "ORF"))
    txs_w_symbols <- merge(txs_wo_symbols, gene_symbols, by.x = "SHORTORF", by.y = "ORF")
  } else {
    gene_symbols <- suppressMessages(AnnotationDbi::select(org_db, keys = genes_in_range$gene_id, columns = assembly$display.column, keytype = assembly$gene.id.column))
