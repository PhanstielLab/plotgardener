...
#'
#' @export
bbPlotHicSquare <- function(data, resolution = "auto", zrange = NULL,
    ...
    ## Define a function that catches errors for bbPlothic
    errorcheck_bbPlothic <- function(hic, hicPlot, norm) {
        ...
        if (!is.null(hicPlot$altchrom)) {
            ...
            bb_regionErrors(chromstart = hicPlot$altchromstart,
    ...
    attr(x = hicPlot, which = "plotted") <- bb_hicInternal$draw
    ...
    if (!is.null(hicPlot$x) & !is.null(hicPlot$y)){
        ...
        if (!identical(width, height)){
            stop("Attempting to plot square Hi-C plot with different width and",
                " height. Use `bbPlotHicTriangle` or `bbPlotHicRectangle` ",
    ...
    if (!is.null(hicPlot$zrange) & length(unique(hicPlot$zrange)) == 2) {
        ...
        hicPlot$color_palette <- bb_hicInternal$palette
    }
    ...
    hicPlot$grobs <- get("hic_grobs", envir = bbEnv)
