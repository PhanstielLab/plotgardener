...
bb_plotgenes <- function(gtf, chrom = "chr8", chromstart = 133600000, chromend = 134800000, strand = "+", col = "black",
  ...
  draw_exon <- function(df, chromstart, chromend, col, fill, gene_height, label, fontsize, fontfamily, fontface){
  ...
  invisible(apply(gtf_subset, 1, draw_exon, chromstart = chromstart, chromend = chromend, col = col, fill = fill,
                  gene_height = gene_height, label = label, fontsize = fontsize, fontfamily = fontfamily, fontface = fontface))
